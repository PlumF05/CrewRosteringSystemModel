...
const form = reactive<SchedulingRules>(
  ...
  <el-card>
    ...
    <el-form label-width="170px" style="max-width: 680px">
      ...
      <el-form-item label="每人最少值班节数">
        <el-input-number
          v-model="form.minSectionsPerAssistant"
          :min="0"
          :max="40"
        />
        节
      ...
      <el-form-item label="每人最多值班节数">
        <el-input-number
          v-model="form.maxSectionsPerAssistant"
          :min="1"
          :max="60"
        />
        节
        <div class="hint">
          如需为个别助理单独设置节数，请在「助理管理」中编辑该助理（个性化值班节数）
        </div>
      </el-form-item>
      ...
      <el-form-item label="上班节次区间">
        <div class="sections">
          ...
          >
          <div class="hint">
            排班只发生在工作日的这些节次内；默认上午 1~4 节、下午 6~9 节
          </div>
        </div>
      </el-form-item>
      ...
      <el-form-item label="排班模式">
        <el-radio-group v-model="form.uniformMode">
          ...
          <el-radio :value="true"
            >各周排班相同（任一周有课即占用，复制到全部周）</el-radio
          >
        </el-radio-group>
      </el-form-item>

      <el-form-item label="课程占用过滤">
        <el-checkbox v-model="form.countTheory"
          >理论课（（本）/（研））计入占用</el-checkbox
        >
        <el-checkbox v-model="form.countExperiment"
          >实验课（（实））计入占用</el-checkbox
        >
      </el-form-item>

      <el-form-item label="最少连续值班节数">
        <el-input-number
          v-model="form.minConsecutiveSections"
          :min="1"
          :max="8"
        />
        节
        <div class="hint">
          大于 1 时，孤立的单节值班不会被安排。例：某助理第 9~10
          节有课、最少连续 2 节时， 第 11 节（其可值班连续段仅剩 1
          节）不会排给他
        </div>
      </el-form-item>
