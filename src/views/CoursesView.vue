...
import { findDutyConflicts, type DutyConflict } from '../algorithms/dutyConflict'
import {
...
function deriveIdentity(courses: ParsedCourse[]): Identity {
  return courses.some((c) => c.identity === 'graduate') ? 'graduate' : 'undergrad'
}
...
async function clearConflicts() {
  const ids = conflicts.value.map((c) => c.id).filter((x): x is number => x !== undefined)
...
async function onFileChange(e: Event) {
  ...
  try {
    ...
    const savedRules = normalizeRules(await configRepo.get<SchedulingRules>('rules'))
    const duties = (await dutyScheduleRepo.all()).filter((d) => d.assistantId === outcome.assistantId)
...
<template>
  ...
  <el-card class="card">
    ...
    <div class="hint">
      ...
      ① 该学号已存在 → 更新其姓名与身份类型（电话 / QQ / 班级保持不动），并导入课程表；<br />
      ② 该学号不存在 → 直接新建助理，并导入课程表。<br />
      两种情况都会<b>整体替换</b>该助理的课程表，重复导入同一文件结果一致（幂等）。
  ...
  <el-card v-if="lastImport" class="card">
    ...
    <el-descriptions :column="4" border size="small">
      <el-descriptions-item label="学生">{{ lastImport.studentName }}</el-descriptions-item>
      <el-descriptions-item label="学号">{{ lastImport.studentNo }}</el-descriptions-item>
      ...
      <el-descriptions-item label="学期">{{ lastImport.semester || '（未识别）' }}</el-descriptions-item>
    </el-descriptions>

    <el-alert
      v-if="lastImport.action === 'updated' && (lastImport.nameChanged || lastImport.identityChanged)"
      type="warning"
      :closable="false"
      class="block"
    ...
    <el-alert v-if="conflicts.length > 0" type="error" :closable="false" class="block">
      <template #title>
        检测到 {{ conflicts.length }} 条已写入排班落在了该生有课的节次上（课程表已更新，建议处理）
      </template>
      <div class="conflict-list">
        <div v-for="c in conflicts" :key="`${c.weekNo}-${c.dayOfWeek}-${c.timeSlot}`">
          第{{ c.weekNo }}周 星期{{ c.dayOfWeek }} 第{{ c.timeSlot.slice(1) }}节 ——
          课程「{{ c.courseName }}」（{{ c.sectionText }}）
        </div>
      </div>
      <div class="conflict-actions">
        <el-button type="danger" size="small" :loading="clearing" @click="clearConflicts">
          删除这些冲突排班
        </el-button>
        <el-button size="small" @click="goReschedule">去排班表重新生成</el-button>
      </div>
    </el-alert>

    <el-table :data="lastImport.courses" border size="small" max-height="360" class="block">
...
.hint { margin-top: 10px; color: var(--el-text-color-secondary); font-size: 12px; line-height: 1.8; }
.block { margin-top: 12px; }
.tag-gap { margin-left: 8px; }
.note { color: var(--el-color-warning); font-size: 12px; }
.conflict-list { font-size: 12px; color: var(--el-text-color-regular); line-height: 1.8; }
